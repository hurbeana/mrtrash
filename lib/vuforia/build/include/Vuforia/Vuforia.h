...
enum PIXEL_FORMAT {
    ...
    NV21 = 64,                  ///< A color pixel stored in 12 or more bits
                                ///< using Y, U and V planes
    NV12 = 128,                 ///< A color pixel stored in 12 or more bits
                                ///< using Y, U and V planes
    YV12 = 256,                 ///< A color pixel stored in 12 or more bits
                                ///< using discreet Y, U and V planes
    YUV420P = 512,              ///< A color pixel stored in 12 or more bits
                                ///< using discreet Y, U and V planes
    YUYV = 1024,                ///< A color pixel stored in 12 or more bits
...
/**
 * \experimental
 * This struct is part of an experimental API and may be changed or removed in future releases.
 *
 * The pointers contained in this struct are owned by Vuforia and should be used
 * with caution by the developer. For example do not release the session, do not
 * pause the session, do not reconfigure it, doing so will cause Vuforia's handling
 * of the information from the provider to fail in undefined ways.
 *
 * A valid value for the session will be available after the camera has been started
 * and it will remain valid until the camera is stopped.
 *
 * A valid value for the frame will be available after the camera has been started
 * and it will only be valid for the duration of one frame. The caller needs to
 * request the information for every new frame.
 *
 * It is expected that these calls will be most useful during the rendering
...
struct FusionProviderPlatformInfo
{
    ...
    {
        mType = FUSION_PROVIDER_PLATFORM_TYPE::FUSION_PROVIDER_PLATFORM_NOT_AVAILABLE;
        mSession = nullptr;
        mFrame = nullptr;
    }
    ...
    /**
     * the caller must cast the mSession pointer to the one appropriate to the
    ...
    /**
     * the caller must cast the mFrame pointer to the one appropriate to the
     * the platform they are on.
     * For ARKit use,
     *
     *     Vuforia::FusionProviderInfo info = Vuforia::getFusionProviderInfo();
     ...
     *
     * In ARCore developers must get the ArFrame using this call. Updating the
     * ArSession to get the ArFrame will cause the Vuforia library to enter an
     * undefined state.
     */
    void *mFrame;
};
 ...
 *
 * The information contained in the returned struct can be used to allow
 * applications to interact with the underlying Platform Fusion provider such
 * as ARKit or ARCore to gain access to functionality not currently available
 * through the Vuforia APIs. For example additional lighting information or
 * plane boundaries. The values contained within the struct are platform
 * specific and need to be used with platform specific code.
 *
 * This API call shoud be made after Vuforia::init(). For instance to use
 * this functionality it can be called in an applications render loop to combine
 * functionality supplied by the underlying Fusion Provider with Vuforia functionality,
 * when the platform specific functionality is not available using Vuforia APIs.
 * see FusionProviderPlatformInfo for more details.
 *
 * \note that some parameters of the returned structure have also a platform specific
 * lifecycle, please see FusionProviderPlatformInfo.
 */
